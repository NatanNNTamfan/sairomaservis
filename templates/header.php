<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>My Store App</title>
    <link href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>
<nav class="navbar navbar-expand-lg navbar-light bg-light">
  <a class="navbar-brand" href="#">My Store App</a>
  <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
    <span class="navbar-toggler-icon"></span>
  </button>
  <div class="collapse navbar-collapse" id="navbarNav">
    <ul class="navbar-nav">
      <li class="nav-item active">
        <a class="nav-link" href="#">Home <span class="sr-only">(current)</span></a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="sales/index.php">Sales</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="stock/index.php">Stock</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="purchase/index.php">Purchase</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="pricing/index.php">Pricing</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="reports/index.php">Reports</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="users/index.php">Users</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="settings/index.php">Settings</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="audit/index.php">Audit</a>
      </li>
    </ul>
  </div>
</nav>
