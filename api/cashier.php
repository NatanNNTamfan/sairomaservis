...
<div class="container mt-4">
    ...
    <form method="post" action="">
        <div class="form-group">
            ...
            <select class="form-control" id="product_id" name="product_id" required>
                ...
                <?php
                $sql = "SELECT id, merk, name, kategori, hargajual FROM products";
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo '<option value="' . $row['id'] . '" data-price="' . $row['hargajual'] . '">' . $row['name'] . ' - Rp ' . number_format($row['hargajual'], 0, ',', '.') . '</option>';
                    }
                } else {
    ...
    <form method="post" action="process_payment.php" onsubmit="return calculateProfit()">
        <input type="hidden" name="products" id="productsInput">
        <button type="submit" class="btn btn-primary">Process Payment</button>
    </form>
    <h3 class="mt-4">Profit Calculation</h3>
    <div id="profitCalculation">
        <p>Total Profit: Rp <span id="totalProfit">0</span></p>
    </div>
</div>
<script>
    let products = [];
    ...
    function updateProductTable() {
        ...
        products.forEach((product, index) => {
            ...
            totalProfit += product.total;
        });

        document.getElementById('totalProfit').innerText = number_format(totalProfit, 0, ',', '.');
